...
[`caseConfig.json`](./caseConfig.json) includes the editable configuration settings, including `reference` and `scaleFactors` dictionaries as well as a `plotABL` boolean and `wDomain` variuable. `reference` includes `fName` of the target ABL, the optimal $h$, $r$, $\alpha$, $k$, and $x$ values, and `hMatch` which I believe only impacts plotting. `scaleFactors` includes the simulation scale, (full)scale building height, and the velocity scaling factor. Note that the simulation scale and building height are really both scaling factors, with the final scaling being `HABL*scale/reference['alpha']`. `wDomain` is 3 by default, but can be modified to match a desired width (w.r.t. unscaled height of 3). If `plotABL` is true, `fName` must match a target ABL profile (./TestCases/{`fName`}.dat). As discussed in the Inflow optimization section, HABL is then calculated as 1.5 times the height of the full-scale building. You also need to specify the testID of the models fitted on the upstream and the downstream databases. 
